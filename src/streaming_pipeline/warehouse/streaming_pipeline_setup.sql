-- Snowflake Setup for Kafka Connect Streaming Pipeline
-- This script sets up the Snowflake environment for Kafka Connect streaming data pipeline
...
-- =====================================================
-- 3. KAFKA CONNECT STAGING TABLES
-- =====================================================

-- These tables will be automatically created by Kafka Connect
-- but we can pre-create them with specific structure if needed

-- Kafka Connect staging table for stock prices
CREATE TABLE IF NOT EXISTS STREAMING.FACT_STOCK_PRICES_STAGING (
    RECORD_METADATA VARIANT,
    RECORD_CONTENT VARIANT
);

-- Kafka Connect staging table for trading volume
CREATE TABLE IF NOT EXISTS STREAMING.FACT_TRADING_VOLUME_STAGING (
    RECORD_METADATA VARIANT,
    RECORD_CONTENT VARIANT
);

-- Kafka Connect staging table for technical indicators
...
-- =====================================================
-- 7. KAFKA CONNECT MONITORING VIEWS
-- =====================================================

-- View to monitor Kafka Connect ingestion
CREATE OR REPLACE VIEW STREAMING.V_KAFKA_CONNECT_MONITORING AS
SELECT 
    'FACT_STOCK_PRICES_STAGING' as TABLE_NAME,
    COUNT(*) as RECORD_COUNT,
    MAX(RECORD_METADATA:CreateTime) as LAST_INGESTION_TIME,
    COUNT(DISTINCT RECORD_METADATA:topic) as TOPIC_COUNT
...
SELECT 
    'FACT_TRADING_VOLUME_STAGING' as TABLE_NAME,
    COUNT(*) as RECORD_COUNT,
    MAX(RECORD_METADATA:CreateTime) as LAST_INGESTION_TIME,
    COUNT(DISTINCT RECORD_METADATA:topic) as TOPIC_COUNT
FROM STREAMING.FACT_TRADING_VOLUME_STAGING
WHERE RECORD_METADATA:CreateTime >= DATEADD(HOUR, -1, CURRENT_TIMESTAMP())

UNION ALL

SELECT 
    'TECHNICAL_INDICATORS_STAGING' as TABLE_NAME,
    COUNT(*) as RECORD_COUNT,
    MAX(RECORD_METADATA:CreateTime) as LAST_INGESTION_TIME,
    COUNT(DISTINCT RECORD_METADATA:topic) as TOPIC_COUNT
FROM STREAMING.TECHNICAL_INDICATORS_STAGING
WHERE RECORD_METADATA:CreateTime >= DATEADD(HOUR, -1, CURRENT_TIMESTAMP());
...
-- =====================================================
-- 12. MONITORING QUERIES FOR KAFKA CONNECT
-- =====================================================

-- Query to check Kafka Connect ingestion status
-- SELECT * FROM STREAMING.V_KAFKA_CONNECT_MONITORING;

-- Query to check recent streaming operations
...
-- Query to check recent data in staging tables
-- SELECT 
...
-- =====================================================
-- SETUP COMPLETE - KAFKA CONNECT READY
-- =====================================================