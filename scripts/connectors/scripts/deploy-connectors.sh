...
deploy_connector "bronze-s3-sink-connector" \
    "scripts/connectors/configs/bronze/bronze-s3-connector.json" \
    "Bronze S3 Connector"
bronze_result=$?

# 2. Deploy Silver S3 Connector  
deploy_connector "silver-s3-sink-connector" \
    "scripts/connectors/configs/silver/silver-s3-connector.json" \
    "Silver S3 Connector"
silver_result=$?
...
if [ ${#missing_vars[@]} -ne 0 ]; then
    ...
    deploy_connector "redshift-streaming-sink" \
        "scripts/connectors/configs/gold/redshift-streaming-connector.json" \
